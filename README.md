...
and company blogs — dedupes and ranks it with an LLM, and sends you a
...
GitHub Actions (cron, 6am IST)
  → fetch: GitHub Search API, arXiv API, HN Algolia API, RSS blogs
...
- **Sources**: add/remove GitHub topics, arXiv categories, RSS feeds
...
- **HN/GitHub APIs are rate-limited** — fine for one daily run, but don't loop
  this to run more frequently.
- **No Reddit source** — Reddit now gates API access behind a manual
  registration/approval process beyond just creating an app; dropped for now
  to keep setup friction-free. Can be re-added if you complete Reddit's
  registration independently (see `git log` for the prior OAuth implementation).
